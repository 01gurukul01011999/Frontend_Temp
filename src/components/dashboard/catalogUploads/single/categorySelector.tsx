...
import categoryTree, { CategoryNode } from '../bulk/categoryTree';
...
function getAllCategoryPaths(tree: Record<string, any>, prefix: string[] = []): string[][] {
  let paths: string[][] = [];
  for (const key in tree) {
    if (tree[key] === null) {
      paths.push([...prefix, key]);
    } else if (typeof tree[key] === 'object') {
      paths = paths.concat(getAllCategoryPaths(tree[key], [...prefix, key]));
    }
  }
  return paths;
}
...
export default function CategorySelector(): React.JSX.Element {
  ...
  React.useEffect(() => {
    if (typeof globalThis !== 'undefined' && !document.getElementById('category-selector-loader-style')) {
...
console.log('uploadimages', uploadedImages);
  ...
  const imageNames = (globalThis as any).uploadedImageNames as string[] | undefined;
    let selectedFiles: File[] = [];
    if (Array.isArray(imageNames) && imageNames.length === uploadedImages.length) {
      selectedFiles = await Promise.all(uploadedImages.map((img, idx) => compressBase64ToFile(img, imageNames[idx])));
    } else {
      // fallback: use previous naming
      selectedFiles = await Promise.all(uploadedImages.map((img, idx) => compressBase64ToFile(img, `image_${idx + 1}.jpg`)));
    }
  ...
  <Box sx={{ display: 'flex', flexDirection: 'row', p:1 , maxheight:'500px', mb:10, overflowY: 'auto', overflowX: 'hidden' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>
        <Autocomplete
          ...
          value={selectionPath.length ? selectionPath : null}
          onChange={(_, newValue) => {
            if (Array.isArray(newValue)) {
              setSelectionPath(newValue);
              setSearchValue(newValue[newValue.length - 1] || '');
            }
          }}
          ...
          renderOption={(props, option, { inputValue }) => {
            ...
            let main, rest;
            if (matchIndex !== -1 && lowerInput.length > 0) {
              main = <>
                {label.substring(0, matchIndex)}
                <b>{label.substring(matchIndex, matchIndex + lowerInput.length)}</b>
                {label.substring(matchIndex + lowerInput.length)}
              </>;
            } else {
              main = label;
            }
          ...
          <Typography variant="body2" sx={{ fontWeight: 600 }}>
            {selectionPath.join(' / ')}
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', mb: 2 }}>
                <img src={url + 'assets/woemns category.png'} style={{ width: '200px' }} />
              </Box>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'row', gap: 3, minHeight: 400 }}>
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={async e => {
                        ...
                        const compressedImgs = await Promise.all(files.map(compressImage));
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mt: 1, width: '100%' }}>
                      {uploadedImages.map((img, idx) => (
                        <Box key={idx} sx={{ position: 'relative', width: 100, height: 100, borderRadius: 2, overflow: 'hidden', border: '1px solid #eee', background: '#fafbff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <img src={img} alt={`uploaded-${idx}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          <IconButton
                            size="small"
          ...
          <Box sx={{ minWidth: 260, maxWidth: 320, pl: 2, borderLeft: '1px solid #eee', display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Typography sx={{ color: '#ff2d2d', fontWeight: 700, mb: 1 }}>
              <span style={{ fontSize: 20, verticalAlign: 'middle', marginRight: 6 }}>🚫</span> Image types which are not allowed:
            </Typography>
              ...
              {[
                ...
                <Box key={item.label} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                  <Box sx={{ width: 50, height: 50, borderRadius: 1, overflow: 'hidden', background: '#f4f4f4', border: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <img src={item.img} alt={item.label} style={{ width: '100%', height:'100%', objectFit: 'contain', filter: 'grayscale(1)' }} />
                  </Box>
                  <Box>
                    <Typography sx={{ fontSize: 15, fontWeight: 500 }}>{item.label}</Typography>
                    <Typography sx={{ fontSize: 12, color: '#888' }}> 🚫 NOT ALLOWED</Typography>
                  </Box>
                </Box>
              ))}
            </Box>
      ...
      <Dialog open={popupOpen} onClose={() => setPopupOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Typography variant="body2" sx={{ mt: 2 }}>
            ✅ Add <strong>Front images</strong> of your product.<br />
            ✅ Get a pre-filled template, quickly add variants, review and enjoy faster catalog bulk uploads.
          </Typography>
        </DialogContent>
        <DialogActions>
